...
![image](https://hackmd.io/_uploads/SkwJ2M9Mel.png)
...
![image](https://hackmd.io/_uploads/B1pc2MqMle.png)

![image](https://hackmd.io/_uploads/H1033f5Mex.png)
...
![image](https://hackmd.io/_uploads/HyrPaGqGgl.png)

Extended opcode 0x51 will read in a byte and write it to the global flag buffer at a specified offset.

![image](https://hackmd.io/_uploads/SkpcaM5Mgl.png)

Extended opcode 0x52 is a little more complicated, but essentially you specify two indexes of the flag buffer as well as an operator. It will then grab the two bytes at their indexes, perform some operation (addition, subtraction, multiplication, xor), and then check the result against a provided value. If this result is incorrect it will change the value of the flag_correct variable.
...
![image](https://hackmd.io/_uploads/S1z0CMcGge.png)

Here is an example 0x51 opcode. The first byte 0x00 signifies an extended opcode, the 0x3 specifies the length of the opcode, 0x51 is the opcode itself, the 0x00 signifies to offset into the flag buffer, and then the char 'd' will be written. This writes the first 'd' of the flag into the buffer.

![image](https://hackmd.io/_uploads/ryUmJQqGxx.png)
...
![image](https://hackmd.io/_uploads/HJpwg75Gll.png)
...
![image](https://hackmd.io/_uploads/SydsxQqGge.png)
...
![image](https://hackmd.io/_uploads/Hk0R-m9fxl.png)
...
![image](https://hackmd.io/_uploads/ByJQTm5Mxe.png)
...
![image](https://hackmd.io/_uploads/rJNcpQcGgl.png)
...
The official solution can be found [here](https://github.com/dicegang/dicectf-quals-2025-challenges/tree/main/pwn/debugapwner) and it's quite clever and uses very minimal bruteforce. In the version of libc used by the challenge in ubuntu 22, `puts` actually resides on the same page as `popen`. Many players get fixated on `system`, but in fact `popen` works great here, though the argument required is more complicated than a `system` call. However, in this case, we have so much control over the (in)correct_msg strings that this is possible. Unfortunately, I did not spot this solution either and found something else.
...
![image](https://hackmd.io/_uploads/ryDVZVqzee.png)
...
![image](https://hackmd.io/_uploads/rJ35z45zxe.png)
...
![image](https://hackmd.io/_uploads/Bk2OSNqGgg.png)
...
![image](https://hackmd.io/_uploads/ByrFP4cGle.png)
...
![image](https://hackmd.io/_uploads/B1IkOV9flg.png)
...
![image](https://hackmd.io/_uploads/B1E2rOqGex.png)
![image](https://hackmd.io/_uploads/HJNyL_cGll.png)
...
![image](https://hackmd.io/_uploads/HJsGL_5Gxl.png)
![image](https://hackmd.io/_uploads/B1_QIu5Mlg.png)
6. `main+413` loads the `stderr` pointer into rdi and executes the unresolved `fprintf` with our controlled address.
![image](https://hackmd.io/_uploads/H1Bv8ucGle.png)
7. `fprintf` calls `puts@plt` which just calls its GOT entry which we already overwritten to point to `gets`.
![image](https://hackmd.io/_uploads/HJShLdcMee.png)
...
![image](https://hackmd.io/_uploads/B1e0FOcGex.png)
10. Shell and flag!
![image](https://hackmd.io/_uploads/r11xqucfge.png)
